...
import { num, fmt, uid, todayISO, isISODate, statusOf, verdicts } from "./engine/format.js";
...
/* Every chart is drawn to the width it will be shown at rather than being
   scrolled sideways, so the drawing has to know which it is. One breakpoint,
   the same one the stylesheet uses. */
function useNarrow() {
...
export default function App() {
  ...
  const says = result ? verdicts(aircraft, status) : [];

  // which boxes to mark: the notice says it, and so does the field itself
  const offChartKeys = new Set(offChart.map((m) => m.key));
  ...
  async function shareCard() {
    ...
    try {
      blob = await view.drawCard({
        aircraft, chart, frame, meta, result, accent: status.hex, says,
  ...
  const groups = useMemo(() => {
    ...
    for (const r of records) {
      const k = seriesKey(r);
      if (!m.has(k)) m.set(k, { key: k, label: seriesLabel(r), aircraft: r.aircraft, rows: [] });
      m.get(k).rows.push(r);
    }
    const out = [...m.values()].sort((a, b) => (a.label < b.label ? -1 : 1));
    /* Two lines may read the same — an untyped registration flown on two
       types, most often UNREG — and picking the wrong one is silent. Where
       that happens the type is named as well. */
    const seen = new Map();
    for (const g of out) seen.set(g.label, (seen.get(g.label) || 0) + 1);
    for (const g of out) if (seen.get(g.label) > 1) g.label = `${byId(g.aircraft).label} · ${g.label}`;
    return out;
  }, [records]);
    ...
    <div className="wrap" style={{ "--accent": status.color }}>
      ...
      <header className="plate">
        ...
        <div className="plate-r">
          ...
          {/* Engine hours are what this check is filed under, not something
              read off a chart, so they sit with the tail and the date. */}
          <label className="hfield">
            <span>Eng hrs</span>
            <input className="hrs" type="number" inputMode="decimal"
      ...
      {tab === "check" && (
        <>
          ...
          <Options opts={fittedOptions(aircraft)} config={config} set={setOption} caption="Fitted" />
          ...
          {/* No example values behind these boxes: a greyed 70.9 sitting in
              an empty torque field reads as a reading that was taken. */}
          <div className="inputs">
            {aircraft.inputs.map((i) => (
              <label key={i.key} className={offChartKeys.has(i.key) ? "field bad" : "field"}>
                <span>{i.label}{i.unit ? ` ${i.unit}` : ""}</span>
                <input type="number" inputMode="decimal" value={values[i.key] ?? ""}
                  aria-invalid={offChartKeys.has(i.key) || undefined}
            ...
            <section className="panel">
              {offChart.length > 0 && (
                <div className="offchart" role="alert">
                  ...
                  {offChart.map((m) => <span key={m.key}>{m.text}</span>)}
                </div>
              )}

              <div className="hero">
                <div className={result ? "big" : "big pending"}>
                  <span>{result ? (result.margin > 0 ? "+" : "") + fmt(result.margin) : "—"}</span><i>{aircraft.marginUnit}</i>
                </div>
                {/* The verdict is said once, in words, below — the number's
                    colour is the same fact at a glance, and printing the
                    status here as well only repeats it. */}
              ...
              {/* The answer in words, not only in colour — the same list the
                  shared card carries. A margin at or above zero is a pass
                  however small it is; the amber band is the operator's
                  caution on top of that, not a different verdict. */}
              {says.map((v) => (
                <p key={v.kind} className={`verdict ${v.kind}`}><b>{v.word}</b>{v.text}</p>
              ))}

              {result && (
                <div className="stats">
                  {result.stats.map((st) => (
                    <div key={st.label}><b>{st.value}</b><span>{st.label}</span></div>
                  ))}
                </div>
              )}
              ...
              <div className="chartwrap">
                <Chart chart={chart} frame={frame} readings={nums} result={result} narrow={narrow} />
              </div>

              <div className="save">
                <input value={note} placeholder="Note" onChange={(e) => setNote(e.target.value)} />
                <button className="btn ghost icon" onClick={shareCard} disabled={!result}>
                ...
                <button className="btn log" disabled={!result} onClick={handleSave}>Log check</button>
              </div>
      ...
      {tab === "trend" && (
        <section className="panel">
          {loading ? <p className="quiet">Loading</p> : groups.length === 0 ? (
            ...
            <>
              {groups.length > 1 && (
                <div className="seg wrapseg" role="group" aria-label="Trend line">
                  {groups.map((g) => (
                    <button key={g.key} className={active && active.key === g.key ? "on" : ""}
                      aria-pressed={active && active.key === g.key}
              ...
              <div className="stats tstats">
                ...
                <div>
                  <b style={{ color: !trend ? "var(--ink-3)" : trend.per < 0 ? "var(--amber)" : "var(--green)" }}>
              ...
              <div className="tchart">
                <ResponsiveContainer width="100%" height={230}>
                  <LineChart data={series} margin={{ top: 10, right: 12, bottom: 0, left: -20 }}>
                    ...
                    <YAxis tick={{ fontSize: 11, fill: "var(--ink-3)" }} stroke="var(--line)" width={42} />
                    <Tooltip contentStyle={{ borderRadius: 2, border: "1px solid var(--line)", fontSize: 12 }}
                      formatter={(v) => [fmt(v) + " " + (seriesAircraft.marginUnit), "Margin"]} />
                    {/* Zero is the chart's own answer and is drawn for every
                        type. A band above it is one operator's practice for
                        one type, so it is drawn only where that type states
                        one — never as a default reaching another's log. */}
                    <ReferenceLine y={0} stroke="var(--red)" />
                    {Number.isFinite(seriesAircraft.watchBelow) && (
                      <ReferenceLine y={seriesAircraft.watchBelow} stroke="var(--amber)" strokeDasharray="4 4" />
                    )}
              ...
              {/* On a phone the readings are dropped rather than pushed off
                  the right edge, taking the delete control with them. They
                  are on the check page and in the export. */}
              <div className="tablewrap">
                <table>
                  <thead>
                    <tr>
                      <th>Date</th><th>Hrs</th>
                      {!narrow && seriesAircraft.inputs.map((i) => <th key={i.key}>{i.label}</th>)}
                      <th>Margin</th><th><span className="sr">Delete</span></th>
                    </tr>
                  </thead>
                  <tbody>
                    {[...series].reverse().map((r) => (
                      <tr key={r.id}>
                        ...
                        {!narrow && seriesAircraft.inputs.map((i) => (
                          <td key={i.key}>{fmt(r[i.key], i.unit === "ft" || i.unit === "°C" ? 0 : 1)}</td>
                        ))}
                        <td style={{ color: statusOf(r.margin, seriesAircraft).color, fontWeight: 700 }}>
                          {(r.margin > 0 ? "+" : "") + fmt(r.margin)}
                        </td>
                        <td className="tx">
                          <button className="x" aria-label={`Delete the check of ${r.date}`}
                            onClick={() => persist(records.filter((q) => q.id !== r.id))}>×</button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
...
/* The margin on this aircraft's own dial.

   Zero is the point the chart itself gives: at or past it the reading was
   over the chart's maximum. An amber band above zero is a judgement, so it
   is drawn only for a type that states one — the same figure that turns the
   number amber, said once more in the same place the number is read. */
function Gauge({ aircraft, margin }) {
  const [from, to] = aircraft.gauge;
  const pct = (v) => ((v - from) / (to - from)) * 100;
  const at = (v) => `${pct(v)}%`;
  const watch = Number.isFinite(aircraft.watchBelow) ? aircraft.watchBelow : null;
  const sign = (v) => (v < 0 ? "−" + Math.abs(v) : String(v));
  return (
    <div className="gauge">
      <div className="gauge-bar">
        <span className="z zr" style={{ width: at(0) }} />
        {watch !== null && <span className="z za" style={{ width: `${pct(watch) - pct(0)}%` }} />}
        <span className="z zg" />
        <i className="pin" style={{ left: at(Math.max(from, Math.min(to, margin))) }} />
      </div>
      <div className="gauge-ticks">
        <span className="t0" style={{ left: at(from) }}>{sign(from)}</span>
        <span style={{ left: at(0) }}>0</span>
        {watch !== null && <span style={{ left: at(watch) }}>{watch}</span>}
        <span className="t1" style={{ left: at(to) }}>{to} {aircraft.marginUnit}</span>
      </div>
    </div>
  );
}
